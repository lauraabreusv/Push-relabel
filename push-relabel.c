...
void PreFluxo(Vertice *grafo_residual, int s, int n){//se precisar por o grafo normal
...
//ver se vai precisar, pq as vezes vai ser melhor fazer enqt busca aumento do fluxo
...
int Push(Vertice *grafo_residual, int v){
	...
	while(aux != NULL){
		if(aux->capacidade - aux->fluxo > 0 && grafo_residual[v].altura > grafo_residual[aux->id_vertice].altura){
			...
			grafo_residual[aux->id_vertice].excesso += fluxo; //sempre q isso rola um push cria excesso no outro
